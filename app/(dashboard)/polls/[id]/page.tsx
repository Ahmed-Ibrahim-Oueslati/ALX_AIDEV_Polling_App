 ...
 * @description Displays the details of a single poll, allowing users to vote or view results.
 * It fetches poll data based on the ID from the URL parameters.
 * @param {{ params: { id: string } }} { params } - The component props, containing the poll ID.
...
export default function PollDetailPage({ params }: { params: { id: string } }) {
  ...
  // In a real application, you would fetch the poll data based on the ID from `params.id`.
  const poll = mockPoll;
  const totalVotes = poll.options.reduce((sum, option) => sum + option.votes, 0);

  /**
   * @function handleVote
   * @description Handles the vote submission process. It simulates an API call and updates the UI to show the results.
   */
  const handleVote = () => {
   ...
   * @description Calculates the percentage of votes for a given option.
   * @param {number} votes - The number of votes for the option.
   * @returns {number} The percentage of votes.
   */
    ...
    <div className="max-w-3xl mx-auto space-y-6">
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          {/* If the user has not voted, show the voting options. */}
          {!hasVoted ? (
            ...
            // If the user has voted, show the poll results.
